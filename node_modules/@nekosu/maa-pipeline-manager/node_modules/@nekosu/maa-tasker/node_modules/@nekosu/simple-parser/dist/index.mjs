//#region src/impl.js
function dump(v) {
	if (typeof v === "object") if (v instanceof Function) return v;
	else if (v instanceof Array) return v.map(dump);
	else {
		const r = {};
		for (const k in v) r[k] = v[k];
		return r;
	}
	else return v;
}
var SimpleParser = class {
					...
					when(...keys) {
						return {
							...
							withloop() {
								const r = rule.for();
								const rr = rule.for();
								rr.when(r.key).do((x) => x).when(r.key, rr.key).do(([x, ys]) => [x, ...ys]);
								keys.push(rr.key);
								return {
									when(...lkeys) {
										return { do: (f) => {
	...
	*/
	canLex(text, rule) {
		...
		if (match && match.index === 0) return [true, match[0].length];
		else return [false, -1];
	}
	/**
	* @param {string} text
	...
	*doLex(text) {
		...
		while (text.length > 0) {
			...
			for (const [name, rule] of this.lexRule.token) {
				if (this.lexRule.tokenFilter?.(`%${name}`, (idx) => {
			...
			if (lastOffset === offset) throw `parse error: lex failed since >>${text.substring(0, 10)}<<`;
			lastOffset = offset;
		}
	}
	...
	*/
	doParse(name, ptr = 0) {
		if (ptr >= this.tokens.length) return [false, []];
		if (name.startsWith("%")) {
	...
	*/
	parse(text) {
		...
		for (const token of this.doLex(text)) this.tokens.push(token);
		this.tokens.push({
			name: "%$end",
			range: [text.length, 0]
		});
		const [state, result] = this.doParse("$");
		if (state && result.length === 1) return result[0][0];
		else throw `parse error: ${result}`;
	}
};
...
function makeParser(token, tokenFilter, _expr, ignore, setup) {

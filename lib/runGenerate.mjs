...
export function substituteText(text, vars) {
  ...
  const stringifyWholePlaceholder = val => {
  ...
  let result = text.replace(/"\s*\$\{(\w+)\}\s*"/g, (match, key) => {
    ...
    return stringifyWholePlaceholder(val)
  })
...
export async function runGenerate(cliConfig = {}, hooks = {}) {
  ...
  if (outputPattern) {
    ...
    for (const entry of dataArray) {
      ...
      const formattedText = formatEnabled
        ? await formatGeneratedOutput(outputText, { baseDir: runtimeBaseDir, filePath })
        : outputText

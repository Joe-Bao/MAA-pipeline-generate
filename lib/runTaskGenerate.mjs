...
import { formatGeneratedOutput } from './formatOutput.mjs'
...
/**
 * 供 generate.mjs 判断：是否应走 task 生成（runTaskGenerate）而非 pipeline（runGenerate）。
 * - `task` / `taskGenerate` / `generateTask` 为 true → task
 * - `pipeline: true` 或 `task: false` → pipeline
 * @param {object} [cfg]
 * @returns {boolean}
 */
export function shouldUseTaskGenerateFromConfig(cfg) {
  if (!cfg || typeof cfg !== 'object') return false
  if (cfg.pipeline === true) return false
  if (cfg.task === false) return false
  return !!(cfg.task === true || cfg.taskGenerate === true || cfg.generateTask === true)
}
...
export async function runTaskGenerate(cliConfig = {}, hooks = {}) {
  ...
  const outputText = formatEnabled
    ? await formatGeneratedOutput(outputRaw, { baseDir: runtimeBaseDir, filePath: targetPath })
    : outputRaw

...
export async function runTaskGenerate(cliConfig = {}, hooks = {}) {
  ...
  const templatePath = cliConfig.template
    ...
    : resolveMaybePath(cfg.taskTemplate, runtimeBaseDir, configDir)
  const dataPath = cliConfig.data
    ? resolve(runtimeBaseDir, cliConfig.data)
    : resolveMaybePath(cfg.taskData, runtimeBaseDir, configDir)
  ...
  const targetPath = cliConfig.target
    ...
    : (
      dataConfig.target
        ? resolve(dirname(dataPath), dataConfig.target)
        : resolveMaybePath(cfg.taskTarget, runtimeBaseDir, configDir)
    )

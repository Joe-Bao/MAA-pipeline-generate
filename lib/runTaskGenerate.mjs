...
function mergeArrayByName(baseArray, nextArray, options) {
  const result = []
  ...
  for (const item of baseArray) {
    ...
    result.push(cloned)
    if (isPlainObject(item) && typeof item.name === 'string' && item.name.length > 0 && !indexByName.has(item.name)) {
      indexByName.set(item.name, result.length - 1)
    }
  }

  for (const item of nextArray) {
    const hasName = isPlainObject(item) && typeof item.name === 'string' && item.name.length > 0
    if (hasName && indexByName.has(item.name)) {
      const idx = indexByName.get(item.name)
      result[idx] = mergeValue(result[idx], item, options)
      continue
    }
...
function mergeArrayValue(baseArray, nextArray, options) {
  if (baseArray.every(v => typeof v === 'string') && nextArray.every(v => typeof v === 'string')) {
    return mergeUniqueStringArray(baseArray, nextArray)
  }

  if (options.objectArrayMerge === 'by-name') {
    const allObjects = [...baseArray, ...nextArray].every(isPlainObject)
    const allHaveName = [...baseArray, ...nextArray].every(v => typeof v.name === 'string' && v.name.length > 0)
    if (allObjects && allHaveName) {
      return mergeArrayByName(baseArray, nextArray, options)
    }
  }

  return [...baseArray.map(cloneValue), ...nextArray.map(cloneValue)]
}

function mergeObjectByKeys(baseObj, nextObj, options) {
  const base = isPlainObject(baseObj) ? baseObj : {}
  const next = isPlainObject(nextObj) ? nextObj : {}
  const out = {}
  const keys = new Set([...Object.keys(base), ...Object.keys(next)])
  for (const key of keys) {
    out[key] = mergeValue(base[key], next[key], options)
  }
  return out
}

function mergeValue(baseValue, nextValue, options) {
  if (nextValue === undefined) return cloneValue(baseValue)
  if (baseValue === undefined) return cloneValue(nextValue)
  if (Array.isArray(baseValue) && Array.isArray(nextValue)) {
    return mergeArrayValue(baseValue, nextValue, options)
  }
  if (isPlainObject(baseValue) && isPlainObject(nextValue)) {
    return mergeObjectByKeys(baseValue, nextValue, options)
  }
  return cloneValue(nextValue)
}

function mergeTaskObject(baseTask, nextTask, options) {
  const merged = mergeValue(baseTask, nextTask, options)
...
function mergeTaskArray(baseTaskArray, fragmentTaskArray, options) {
  ...
  if (Array.isArray(fragmentTaskArray)) {
    for (const task of fragmentTaskArray) {
      ...
      map.set(key, mergeTaskObject(existing, task, options))
    }
  }
...
function mergeOptionObject(baseOption, fragmentOption, options) {
  return mergeObjectByKeys(baseOption, fragmentOption, options)
}
...
function mergeTaskFragment(baseDoc, fragmentDoc) {
  ...
  const mergeOptions = { objectArrayMerge: 'by-name' }
  const merged = {
    ...mergeValue(base, fragment, mergeOptions)
  }
  merged.task = mergeTaskArray(base.task, fragment.task, mergeOptions)
  merged.option = mergeOptionObject(base.option, fragment.option, mergeOptions)
  return merged
}
 ...
 * @returns {'on'|'off'}
 */
function parseTaskModeValue(raw) {
  if (raw === undefined || raw === null || raw === '') return 'on'
  if (typeof raw === 'boolean') return raw ? 'on' : 'off'
  if (typeof raw === 'number' && Number.isFinite(raw)) return raw === 0 ? 'off' : 'on'
  if (typeof raw === 'string') {
    const s = raw.trim().toLowerCase()
    if (['on', 'true', 'yes', '1', 'smart'].includes(s)) return 'on'
    if (['off', 'false', 'no', '0'].includes(s)) return 'off'
    if (['legacy', 'old'].includes(s)) {
      throw new Error('[task-generate] taskMergeMode: legacy 已移除，请使用 taskmode: on 或删除该字段（默认即为深度增量合并）')
    }
    throw new Error(`[task-generate] 无法识别 taskmode / taskMergeMode: ${raw}`)
  }
  throw new Error('[task-generate] taskmode 类型无效')
}

/**
 * @param {'on'|'off'} mode
 */
function assertTaskModeOn(mode) {
  if (mode === 'off') {
    throw new Error('[task-generate] taskmode 已关闭：task 生成仅支持深度增量合并（taskmode: on），旧式浅合并已移除')
  }
}
 ...
 * @param {string|boolean|number} [cliConfig.taskmode]
 * @param {string|boolean|number} [cliConfig.taskMode]
 * @param {string|boolean|number} [cliConfig.taskMergeMode]
 * @param {object} [hooks]
...
export async function runTaskGenerate(cliConfig = {}, hooks = {}) {
  ...
  const configLegacyTarget = pickFirstDefined(cfg.taskTarget, cfg.target)
  const configOutputFile = pickFirstDefined(cfg.outputFile, cfg.taskOutputFile)
  ...
  if (!templatePath || !dataPath) {
    throw new Error('[task-generate] 缺少必要参数，请提供 --template 与 --data（或在 config.json 中配置 template/data）')
  }
  ...
  const taskModeRaw = pickFirstDefined(
    cliConfig.taskmode,
    cliConfig.taskMode,
    cliConfig.taskMergeMode,
    dataConfig.taskmode,
    dataConfig.taskMode,
    dataConfig.taskMergeMode,
    cfg.taskmode,
    cfg.taskMode,
    cfg.taskMergeMode
  )
  const taskMode = parseTaskModeValue(taskModeRaw)
  assertTaskModeOn(taskMode)
  emit('log', `[task-generate] 模板类型: ${templateKind}`)
  emit('log', `[task-generate] taskmode: ${taskMode}`)
  const outputDir = resolve(runtimeBaseDir, cliConfig.outputDir ?? cfg.outputDir ?? dataConfig.outputDir ?? 'output')
  const dataLegacyTarget = pickFirstDefined(dataConfig.taskTarget, dataConfig.target)
  const dataOutputFile = pickFirstDefined(dataConfig.outputFile, dataConfig.taskOutputFile)

  const cliTargetPath = cliConfig.target ? resolve(runtimeBaseDir, cliConfig.target) : null
  const cliOutputFilePath = cliConfig.outputFile
    ? (isAbsolute(cliConfig.outputFile) ? cliConfig.outputFile : resolve(outputDir, cliConfig.outputFile))
    : null
  const configOutputFilePath = configOutputFile
    ? (isAbsolute(configOutputFile) ? configOutputFile : resolve(outputDir, configOutputFile))
    : null
  const legacyConfigTargetPath = configLegacyTarget
    ? resolveMaybePath(configLegacyTarget, runtimeBaseDir, configDir)
    : null
  const dataOutputFilePath = dataOutputFile
    ? (isAbsolute(dataOutputFile) ? dataOutputFile : resolve(outputDir, dataOutputFile))
    : null
  const legacyDataTargetPath = dataLegacyTarget
    ? (isAbsolute(dataLegacyTarget) ? dataLegacyTarget : resolve(dirname(dataPath), dataLegacyTarget))
    : null

  // data 内 target/outputFile 优先于 config：避免与 pipeline 共用的 config.json 里 outputFile 误伤 task 生成路径。
  const targetPath = pickFirstDefined(
    cliTargetPath,
    cliOutputFilePath,
    dataOutputFilePath,
    legacyDataTargetPath,
    configOutputFilePath,
    legacyConfigTargetPath
  )

  if (!targetPath) {
    throw new Error('[task-generate] 缺少目标文件，请提供 --target / --output-file（或在 data/config 中配置 outputFile / target）')
  }

  emit('log', `[task-generate] 输出目录: ${outputDir}`)
